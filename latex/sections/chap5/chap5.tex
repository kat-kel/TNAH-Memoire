...
L'arborescence actuelle du format \acrshort{ALTO} diffère de l'original car, aujourd'hui, elle peut préciser les coordonnées d'un polygone en plus d'un rectangle. Cela est un développement dans la technologie des logiciels \acrshort{OCR} et \acrshort{HTR}. La visualisation dans la Figure~\ref{fig:newstructure} montre le nouvel élément \texttt{<Polygon>} qui descend directement d'un élément (\texttt{<Shape>}) qui lui-même ne porte pas d'attribut ni d'intérêt dans l'arborescence que de baliser les informations du polygone. Le contenu textuel est toujours balisé dans l'élément \texttt{<String>}, qui porte sur un segment ou mot d'une ligne de texte (\texttt{<TextLine>}). Mais en allant jusqu'au détail du glyphe dans l'arborescence actuelle, l'élément \texttt{<Glyph>} représente tout caractère composant un mot (\texttt{<String>}).
