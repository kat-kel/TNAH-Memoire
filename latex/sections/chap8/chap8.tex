...
\end{lstlisting}
\caption{Le \texttt{<TextLine>} en \acrshort{ALTO}}
\label{alto:line}
\end{subfigure}
...
Pour certains fichiers \acrshort{ALTO}, tel que ceux qui sortent de l'interface \textit{eScriptorium}, la modélisation en \acrshort{TEI} s'arrête là. Le fichier \acrshort{ALTO} ne porte pas de plus de détail après la ligne de texte. Mais pour certains d'autres fichiers, tel que ceux qui sont produits par l'engin \textit{Kraken} depuis la ligne de commande, ils attestent aux prédictions sur des mots et sur des glyphes. Dans ce cas, la ligne de texte a plus de descendants, mais afin de garder une arborescence générique qui supporte des comparaisons entre des fichiers de divers formats, le \texttt{<zone>} de la ligne de texte garde toujours les mêmes deux enfants~: le \texttt{<path>} et le \texttt{<line>}. En plus de ces deux, le \texttt{<zone>} contient une suite d'éléments \texttt{<zone>} pour tout segment prédit sur la ligne, soit un mot, soit une espace entre mots.

% Segment
\subsection{Le segment}
Les fichiers \acrshort{ALTO} qui contiennent plus de détail ont des éléments \texttt{<zone>} pour des segments (\texttt{<String>}) prédits et pour les glyphes (\texttt{<Glyph>}) prédits dans les segment. Les modèles \acrshort{HTR} prédisent les segments qui contiennent des glyphes, tel qu'un mot, et ceux qui n'en ont pas, tel qu'une espace entre mots. Les segment qui contiennent des glyphes peuvent représenter la prédiction d'un mot, de la ponctuation, ou d'un mot avec de la ponctuation à côté. L'important est que le segment contient soit la prédiction d'au moins un glyphe ou la prédiction d'une espace entre mots. Uniquement les segments (\texttt{<String>}) qui contiennent des prédictions sur des glyphes portent aussi un polygone dans lequel s'encadre la chaîne des glyphes. Si le \texttt{<String>} représente une espace entre mots, le masque prédit n'est qu'un rectangle. Bien que l'élément \texttt{<String>} ne porte pas directement sur le texte prédit, le modèle \acrshort{HTR} évalue son taux de réussite à partir des glyphes bien prédits dedans. L'évaluation de sa prédiction est représentée dans l'attribut \texttt{@WC} de l'élément \texttt{<String>}. L'acronyme \textit{WC} signifie en anglais \textit{word confidence}. L'exemple du \texttt{<String>} et l'exemple de sa modélisation en \acrshort{TEI} sont donnés dans la Figure~\ref{fig:string}. Une visualisation de la transformation d'\acrshort{ALTO} à \acrshort{TEI} est donnée dans la Figure~\ref{}.


% Glyph
\subsection{Le glyphe}
Les caractères et de la ponctuation prédits par des modèles \acrshort{HTR} sont tous encodés dans l'élément \texttt{<Glyph>} selon le schème \acrshort{ALTO}. Contrairement au \texttt{<String>} qui sert à contenir une chaîne de glyphes, le \texttt{<Glyph>} du fichiers \acrshort{ALTO} contient à la fois le masque et le texte. Pour cette raison, la modélisation en \acrshort{TEI} représente, comme d'habitude, le masque du \texttt{<Glyph>} dans l'élément \texttt{<zone>} et le texte prédit dans l'élément \texttt{<c>}. Ce dernier est un élément du schème \acrshort{TEI} destiné à la représentation d'un caractère, soit une lettre, soit de la ponctuation. Il convient bien donc à la représentation de toute prédiction dans l'élément \texttt{<Glyph>} du fichier \acrshort{ALTO}. Le modèle \acrshort{HTR} évalue son taux de réussite de sa prédiction du glyphe. L'évaluation de sa prédiction est représentée dans l'attribut \texttt{@GC} de l'élément \texttt{<String>}. L'acronyme \textit{GC} signifie en anglais \textit{glyph confidence}. L'exemple du \texttt{<Glyph>} et l'exemple de sa modélisation en \acrshort{TEI} sont donnés dans la Figure~\ref{fig:}. Une visualisation de la transformation d'\acrshort{ALTO} à \acrshort{TEI} est donnée dans la Figure~\ref{}.


\section{Les visualisations de la transformation}
Des visualisations de la modélisation de chaque élément du fichier \acrshort{ALTO} sont montrées dans les figures qui suivent. Les attributs sont visualisés par les carrés en ligne tirée et les éléments \acrshort{XML} sont visualisés par les carrés en ligne solide. La couleur de l'élément ou de l'attribut du schème \acrshort{ALTO} est répétée dans la visualisation du schème \acrshort{TEI} quand sa valeur est utilisée.
% empty
...
\end{tikzpicture}
\caption{Le modèle du \texttt{<surface>} en \acrshort{TEI}}
\label{mod:pageTEI}
\end{subfigure}

\caption{La transformation de la page d'\acrshort{ALTO} à \acrshort{TEI}}
\label{fig:modPage}
\end{figure}
...
\begin{lstlisting}[language=XML]
\end{lstlisting}
\caption{Le \texttt{<zone>} en \acrshort{TEI}}
\label{tei:string}
\end{subfigure}
...
\begin{lstlisting}[language=XML]
\end{lstlisting}
\caption{Le \texttt{<>} en \acrshort{ALTO}}
\label{alto:}
\end{subfigure}
...
\begin{lstlisting}[language=XML]
\end{lstlisting}
\caption{Le \texttt{<>} en \acrshort{TEI}}
\label{tei:}
\end{subfigure}
\caption{La modélisation du \texttt{<>}}
\label{fig:}
\end{figure}
